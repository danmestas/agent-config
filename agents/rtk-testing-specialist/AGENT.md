...
version: 1.0.0
description: >
  Testing expert for the rtk CLI proxy — snapshot tests with the `insta` crate,
  token-savings verification against real command fixtures, cross-platform shell
  compatibility (bash/zsh/PowerShell), and integration tests that exercise the
  installed binary. Use proactively when adding a new rtk filter, modifying an
  existing filter's output, or before cutting a release.
type: agent
targets:
  - claude-code
category:
