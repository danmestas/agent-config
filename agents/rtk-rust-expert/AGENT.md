...
version: 1.0.0
description: >
  Expert Rust developer for the rtk CLI proxy — CLI proxy patterns, filter
  design, regex compilation with `lazy_static!`, zero-overhead startup
  (<10ms), graceful fallback on filter failure, anyhow-based error
  handling with `.context()` on every `?`, and cross-platform shell
  escaping. Use when implementing a new rtk filter, refactoring an
  existing one, or reviewing rtk Rust code for performance or correctness
  regressions.
type: agent
targets:
  - claude-code
