...
targets: [claude-code]
type: skill
description: Use when adding a new rtk filter (e.g. `rtk newcmd`) or fixing a failing filter test — runs the full Red-Green-Refactor loop with real command fixtures, token-savings assertions (>= 60%), and `insta` snapshot tests
category:
  primary: workflow
license:
  upstream: Apache-2.0
...
// WRONG: Regex inside the filter function
...
- `skills/rtk-tdd/` - language-agnostic TDD discipline for rtk Rust code (no filter-specific scaffolding)
- `skills/rtk-tdd-rust/references/testing-patterns.md` - the backlog of untested rtk modules and the four canonical test patterns
