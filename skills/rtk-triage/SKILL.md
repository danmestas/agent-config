...
targets: [claude-code]
type: skill
description: 'Use when you want a full rtk repo health pass that runs `/rtk-issue-triage` and `/rtk-pr-triage` in parallel and then cross-references the results — surfaces double-coverage (two PRs for one issue), security findings without PRs, P0/P1 bugs without PRs, our dirty PRs with probable causes, and untracked PRs. Saves the report to `claudedocs/RTK-YYYY-MM-DD.md`. Args: `en`/`fr` (default fr), `save`/`no save`.'
category:
  primary: workflow
license:
  upstream: Apache-2.0
...
Orchestrateur de triage RTK. Fusionne `/rtk-issue-triage` + `/rtk-pr-triage` et produit une analyse croisée.
...
Exécuter les analyses de `/rtk-issue-triage` et `/rtk-pr-triage` séparément (même logique que les skills individuels) pour produire :
...
Afficher les tableaux standards de chaque skill (voir SKILL.md de `rtk-issue-triage` et `rtk-pr-triage` pour le format exact).
...
| **Description finding critique** | **AUCUNE** | Trou |
```
...
Croisement issues x PRs. {N} PRs ouvertes, {N} issues ouvertes.
