...
| Skill | Usage | Output |
|-------|-------|--------|
| `/rtk-issue-triage` | Trier, analyser, commenter les issues | Tableaux d'action + deep analysis + commentaires postés |
| `/rtk-repo-recap` | Récap général pour partager avec l'équipe | Résumé Markdown (PRs + issues + releases) |

**Déclencheurs** :
- Manuellement : `/rtk-issue-triage` ou `/rtk-issue-triage all` ou `/rtk-issue-triage 42 57`
